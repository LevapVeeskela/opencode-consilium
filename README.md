...
ОПЦИИ:
  ...
  -c, --config <файл>     Путь к конфигурации
  -e, --experts <список>  Эксперты через запятую
  --chair <агент>         Агент-председатель

  --expert-timeout <мс>   Таймаут экспертов
  --chair-timeout <мс>    Таймаут председателя

  -o, --output <файл>     Выходной файл
  -f, --format <формат>   Формат: text, markdown, json
  -l, --log-level <ур.>   Уровень логов: silent, error, warn, info, debug

  -r, --retries <число>   Повторные попытки

КОМАНДЫ:
  --init                  Создать пример конфига
  --list-agents           Показать агентов
  --dry-run               Показать промпты без вызова
```
